    ...
    : Comparable<SolveTime>
{
    ...
    companion object
    {
        // String extension function toDuration()
        fun String.toDuration(): Duration {

            val durationStr = StringBuilder("PT")

            val colonsCount = this.filter { it == ':' }.count() // counts the number of colons in the input string
        ...
        decimalsStr.length.let {
            when {
                it > 2-> {
                    ...
                    if(decimalsStr[0] == '0') {
                        timeStr = timeStr.replaceAfterLast(".", "0$decimals")
                    } else {
                        timeStr = timeStr.replaceAfterLast( ".", decimals.toString())
                    }
                }
...
} // calculates the mean of a list of solves times